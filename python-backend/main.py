...
# ✅ H10 处理模块
try:
    from h10_processor import process_h10_optimization
except ImportError:
    # 如果导入失败，创建一个占位符函数
    async def process_h10_optimization(*args, **kwargs):
        raise HTTPException(status_code=500, detail="H10 优化处理模块未正确导入")
...
async def process_excel(
    ...
    H10_OPTIMIZATION_SERVICE_ID = "3b23604d-7b49-4547-b866-d376ec0fefba"
    
    # ✅ H10 服务特殊处理
    if service_id == H10_OPTIMIZATION_SERVICE_ID:
        try:
            result_bytes = await process_h10_optimization(
